...
impl Decoder {
        ...
        Func: Fn(Action),
    {
        ...
        match self.command {
            ...
            'A' => on_action(Action::cursor_movement(
                action::CursorDirection::Up,
                def_or_zero(1),
            )),
            'B' => on_action(Action::cursor_movement(
                action::CursorDirection::Down,
                def_or_zero(1),
            )),
            'C' => on_action(Action::cursor_movement(
                action::CursorDirection::Right,
                def_or_zero(1),
            )),
            'D' => on_action(Action::cursor_movement(
                action::CursorDirection::Left,
                def_or_zero(1),
            )),
            ...
            'c' => {
                assert!(def(0) == 0);
                on_action(Action::response("\x1b[1;2c".as_bytes().to_vec()));
            }
    ...
    fn finish_bracket(&mut self) {
        match self.command {
            _ => {
                if SHOW_UNKOWN_ESCAPES {
                    println!("Unkown bracket escape {}", self.command);
                }
            }
        }
        ...
        Func: Fn(Action),
    {
        ...
        loop {
            ...
            match self.state {
                State::Ascii => {
                    if (self.c as u32 & 0b11100000) == 0b11000000 {
                        ...
                        match self.c {
                            ...
                            '\x07' => println!("Bell!!!"),
                            '\x08' => {
                                on_action(Action::cursor_movement(action::CursorDirection::Left, 1))
                            }
                ...
                State::CommandBody => match self.c {
                    '\x07' => {
                        self.finish_command();
                        self.state = State::Ascii;
                    }
