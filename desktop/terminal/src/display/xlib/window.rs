...
impl Window {
    pub fn open(width: i32, height: i32, title: &str) -> Rc<RefCell<Self>> {
        ...
        if unsafe { xlib::XMatchVisualInfo(display, screen, 24, xlib::TrueColor, &mut visual_info) }
            == 0
        {
        ...
        let cmap = unsafe { xlib::XCreateColormap(display, root_window, visual, xlib::AllocNone) };
