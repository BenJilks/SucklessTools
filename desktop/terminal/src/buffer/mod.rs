...
impl Line {
    ...
    pub fn resize(&mut self, columns: i32) {
        ...
        self.content
            .resize(columns as usize, LineElement::default());
...
pub struct Buffer {
    ...
    cursor: CursorPos,
    attribute: Attribute,
    draw_action_sender: mpsc::Sender<DrawAction>,
...
impl Buffer {
    pub fn new(rows: i32, columns: i32, draw_action_sender: mpsc::Sender<DrawAction>) -> Self {
        let mut buffer = Self {
    ...
    pub fn redraw(&mut self) {
        let _ = self.draw_action_sender.send(DrawAction::ClearScreen);
    ...
    fn scroll_down(&mut self, mut amount: i32, top: i32, bottom: i32) {
        ...
        self.scrollback
            .append(&mut self.content[old_start_index..old_end_index].to_vec());
    ...
    pub fn reset_viewport(&mut self) {
        self.selection_end();
        let _ = self.draw_action_sender.send(DrawAction::ResetViewport);
        self.redraw();
    }

    fn out_of_bounds(&self, pos: &CursorPos) -> bool {
        return pos.get_row() < 0
            || pos.get_row() >= self.rows
            || pos.get_column() < 0
            || pos.get_column() >= self.columns;
    }
    ...
    fn cursor_left(&mut self, amount: i32) {
        self.cursor_move(0, -amount)
    }
    fn cursor_right(&mut self, amount: i32) {
        self.cursor_move(0, amount)
    }
    fn cursor_up(&mut self, amount: i32) {
        self.cursor_move(-amount, 0)
    }
    fn cursor_down(&mut self, amount: i32) {
        self.cursor_move(amount, 0)
    }
    ...
    fn clear_block_range(&mut self, start: i32, end: i32) {
        if !(0..self.rows).contains(&start) || !(0..=self.rows).contains(&end) {
            return;
        }
    ...
    pub fn do_action(&mut self, action: Action) -> Vec<u8> {
        match action.action_type {
            ...
            ActionType::CursorMovement => match action.cursor_direction.unwrap() {
            ...
            ActionType::ClearFromCursor => match action.cursor_direction.unwrap() {
            ...
            ActionType::SetColor => {
                *self.attribute.from_type(&action.color_type.unwrap()) = action.color.unwrap()
            }
